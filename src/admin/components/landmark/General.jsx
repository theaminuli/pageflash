...
import { useGetLandmarks, usePageflashContext, usePutLandmarkSlug } from "../../hooks";
...
const General = () => {
	...
	return (
		<HStack alignment="normal" direction="column" spacing={5} className="pageflash-general">
			{
				data.map((item) => {
					if (item.type === 'switch') {
						return (
							<Switch
								...
								onToggle={(newValue) => handleChange(newValue, item.slug)}
							/>
						);
					}
					if (item.type === 'text') {
