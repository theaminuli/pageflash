...
async function updateVersion() {
	...
	if ( ! fs.existsSync( './pageflash.php' ) ) {
		console.error( '❌ Error: pageflash.php file not found.' );
		process.exit( 1 );
	}
	...
	// update pageflash.php version
	const pluginPhpFile = fs.readFileSync( './pageflash.php', 'utf8' );
	const newPluginPhpFile = pluginPhpFile.replace(
		/Version: (.*)/,
		`Version: ${ newVersion }`
	);
	fs.writeFileSync( './pageflash.php', newPluginPhpFile );
	console.info(
		'✅  pageflash.php file version updated',
		currentTag,
		'=>',
		newTag
	);
